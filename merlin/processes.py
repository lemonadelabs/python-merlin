...
class GeneratorProcess(merlin.Process):
    ...
    Args:
        ...
        store (float): the maximum units that can be produced by this generator in the simulation. Set to -1 to allow infinite supply.

    Kwargs:
        name (str): An optional human-readible name for the generator instance
        restrict ([str]): a list of connection outputs to exclude.

        production_per_timedelta (timedelta): if this is not None, generator  will produce ((time_interval/timedelta) * unit_rate). If None, it will produce unit_rate every tick.
    ...
    def _get_unit():
        if self.prod_per_t:
            output = (self.parent.sim.current_time_interval.total_seconds()/self.prod_per_t.total_seconds()) * self.unit_rate
        else:
